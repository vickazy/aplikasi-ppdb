...
function &DbHelper($dbid = "") {
	if ($dbid == "pepedebe1" || $dbid === 1) // pepedebe1
		$dbclass = "cpepedebe1_db";
	else // DB
		$dbclass = "cpepedebe_db";
...
function CurrentProjectID() {
	...
	return "{27E315CC-B02A-4AE4-92E7-B346521044C4}";
}
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", "HomePage", "index2.php", "ewHome", "", FALSE); // Home
	}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ew_ComparePassword($rs->fields('pwd'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('nama_ruang')); // Load user name
						$this->setSessionUserID($rs->fields('nama_ruang')); // Load User ID
						$this->setSessionParentUserID($rs->fields('nama_ruang')); // Load parent User ID
						...
						$UserProfile->Delete('pwd'); // Delete password
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $UserTableConn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('nama_ruang');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('nama_ruang'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`nama_ruang` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('nama_ruang'));

...
//
// Table class for no_pendaftaran
//
class cno_pendaftaran extends cTable {
	var $id_no;
	...
	function __construct() {
		...
		$this->TableVar = 'no_pendaftaran';
		$this->TableName = 'no_pendaftaran';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`no_pendaftaran`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_no
		$this->id_no = new cField('no_pendaftaran', 'no_pendaftaran', 'x_id_no', 'id_no', '`id_no`', '`id_no`', 3, -1, FALSE, '`id_no`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_no->Sortable = TRUE; // Allow sort
		$this->id_no->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_no'] = &$this->id_no;

		// nomor_pendaftaran
		$this->nomor_pendaftaran = new cField('no_pendaftaran', 'no_pendaftaran', 'x_nomor_pendaftaran', 'nomor_pendaftaran', '`nomor_pendaftaran`', '`nomor_pendaftaran`', 200, -1, FALSE, '`nomor_pendaftaran`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nomor_pendaftaran->Sortable = TRUE; // Allow sort
		$this->fields['nomor_pendaftaran'] = &$this->nomor_pendaftaran;

		// nama_ruang
		$this->nama_ruang = new cField('no_pendaftaran', 'no_pendaftaran', 'x_nama_ruang', 'nama_ruang', '`nama_ruang`', '`nama_ruang`', 200, -1, FALSE, '`nama_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`no_pendaftaran`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_no->setDbValue($conn->Insert_ID());
			$rs['id_no'] = $this->id_no->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_no', $rs))
				ew_AddFilter($where, ew_QuotedName('id_no', $this->DBID) . '=' . ew_QuotedValue($rs['id_no'], $this->id_no->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_no->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_no->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_no@", ew_AdjustSql($this->id_no->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "no_pendaftaranedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "no_pendaftaranadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("no_pendaftaranview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("no_pendaftaranview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "no_pendaftaranadd.php?" . $this->UrlParm($parm);
		else
			$url = "no_pendaftaranadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("no_pendaftaranedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("no_pendaftaranadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("no_pendaftarandelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_no:" . ew_VarToJson($this->id_no->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_no->CurrentValue)) {
			$sUrl .= "id_no=" . urlencode($this->id_no->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->id_no->Exportable) $Doc->ExportCaption($this->id_no);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->id_no->Exportable) $Doc->ExportField($this->id_no);
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		...
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `no_pendaftaran`";

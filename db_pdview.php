...
$db_pd_view = NULL; // Initialize page object first

class cdb_pd_view extends cdb_pd {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["id_pd"] <> "") {
			$this->RecKey["id_pd"] = $_GET["id_pd"];
			$KeyUrl .= "&amp;id_pd=" . urlencode($this->RecKey["id_pd"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_pd"] <> "") {
				...
				$this->RecKey["id_pd"] = $this->id_pd->QueryStringValue;
			} elseif (@$_POST["id_pd"] <> "") {
				$this->id_pd->setFormValue($_POST["id_pd"]);
				$this->RecKey["id_pd"] = $this->id_pd->FormValue;
			} else {
				$sReturnUrl = "db_pdlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "db_pdlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "db_pdlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_db_pd\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_db_pd',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fdb_pdview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($db_pd_view)) $db_pd_view = new cdb_pd_view();

// Page init
$db_pd_view->Page_Init();

// Page main
$db_pd_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$db_pd_view->Page_Render();
?>
...
var CurrentForm = fdb_pdview = new ew_Form("fdb_pdview", "view");

// Form_CustomValidate event
fdb_pdview.Form_CustomValidate = 
...
fdb_pdview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if ($db_pd->Export == "") { ?>
<div class="ewToolbar">
<?php $db_pd_view->ExportOptions->Render("body") ?>
<?php
	foreach ($db_pd_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $db_pd_view->ShowPageHeader(); ?>
<?php
$db_pd_view->ShowMessage();
?>
<form name="fdb_pdview" id="fdb_pdview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($db_pd_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $db_pd_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="db_pd">
<input type="hidden" name="modal" value="<?php echo intval($db_pd_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($db_pd->id_pd->Visible) { // id_pd ?>
	<tr id="r_id_pd">
		<td class="col-sm-2"><span id="elh_db_pd_id_pd"><?php echo $db_pd->id_pd->FldCaption() ?></span></td>
		<td data-name="id_pd"<?php echo $db_pd->id_pd->CellAttributes() ?>>
<span id="el_db_pd_id_pd">
<span<?php echo $db_pd->id_pd->ViewAttributes() ?>>
<?php echo $db_pd->id_pd->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->nopes->Visible) { // nopes ?>
	<tr id="r_nopes">
		<td class="col-sm-2"><span id="elh_db_pd_nopes"><?php echo $db_pd->nopes->FldCaption() ?></span></td>
		<td data-name="nopes"<?php echo $db_pd->nopes->CellAttributes() ?>>
<span id="el_db_pd_nopes">
<span<?php echo $db_pd->nopes->ViewAttributes() ?>>
<?php echo $db_pd->nopes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->nm_pes->Visible) { // nm_pes ?>
	<tr id="r_nm_pes">
		<td class="col-sm-2"><span id="elh_db_pd_nm_pes"><?php echo $db_pd->nm_pes->FldCaption() ?></span></td>
		<td data-name="nm_pes"<?php echo $db_pd->nm_pes->CellAttributes() ?>>
<span id="el_db_pd_nm_pes">
<span<?php echo $db_pd->nm_pes->ViewAttributes() ?>>
<?php echo $db_pd->nm_pes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->nisn->Visible) { // nisn ?>
	<tr id="r_nisn">
		<td class="col-sm-2"><span id="elh_db_pd_nisn"><?php echo $db_pd->nisn->FldCaption() ?></span></td>
		<td data-name="nisn"<?php echo $db_pd->nisn->CellAttributes() ?>>
<span id="el_db_pd_nisn">
<span<?php echo $db_pd->nisn->ViewAttributes() ?>>
<?php echo $db_pd->nisn->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->asal_sek->Visible) { // asal_sek ?>
	<tr id="r_asal_sek">
		<td class="col-sm-2"><span id="elh_db_pd_asal_sek"><?php echo $db_pd->asal_sek->FldCaption() ?></span></td>
...
<?php echo $db_pd->asal_sek->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->n_ind->Visible) { // n_ind ?>
	<tr id="r_n_ind">
		<td class="col-sm-2"><span id="elh_db_pd_n_ind"><?php echo $db_pd->n_ind->FldCaption() ?></span></td>
		<td data-name="n_ind"<?php echo $db_pd->n_ind->CellAttributes() ?>>
<span id="el_db_pd_n_ind">
<span<?php echo $db_pd->n_ind->ViewAttributes() ?>>
<?php echo $db_pd->n_ind->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->n_mat->Visible) { // n_mat ?>
	<tr id="r_n_mat">
		<td class="col-sm-2"><span id="elh_db_pd_n_mat"><?php echo $db_pd->n_mat->FldCaption() ?></span></td>
		<td data-name="n_mat"<?php echo $db_pd->n_mat->CellAttributes() ?>>
<span id="el_db_pd_n_mat">
<span<?php echo $db_pd->n_mat->ViewAttributes() ?>>
<?php echo $db_pd->n_mat->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->n_ipa->Visible) { // n_ipa ?>
	<tr id="r_n_ipa">
		<td class="col-sm-2"><span id="elh_db_pd_n_ipa"><?php echo $db_pd->n_ipa->FldCaption() ?></span></td>
		<td data-name="n_ipa"<?php echo $db_pd->n_ipa->CellAttributes() ?>>
<span id="el_db_pd_n_ipa">
<span<?php echo $db_pd->n_ipa->ViewAttributes() ?>>
<?php echo $db_pd->n_ipa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($db_pd->n_jml->Visible) { // n_jml ?>
	<tr id="r_n_jml">
		<td class="col-sm-2"><span id="elh_db_pd_n_jml"><?php echo $db_pd->n_jml->FldCaption() ?></span></td>
		<td data-name="n_jml"<?php echo $db_pd->n_jml->CellAttributes() ?>>
<span id="el_db_pd_n_jml">
<span<?php echo $db_pd->n_jml->ViewAttributes() ?>>
<?php echo $db_pd->n_jml->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>

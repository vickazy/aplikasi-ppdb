...
$pendaftar_list = NULL; // Initialize page object first

class cpendaftar_list extends cpendaftar {
	...
	// Page object name
	var $PageObjName = 'pendaftar_list';

	// Grid form hidden field names
	var $FormName = 'fpendaftarlist';
	...
	function __construct() {
		...
		$this->AddUrl = "pendaftaradd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "pendaftardelete.php";
		$this->MultiUpdateUrl = "pendaftarupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpendaftarlistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fpendaftarlistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fpendaftarlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fpendaftarlist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpendaftarlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpendaftarlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fpendaftarlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpendaftarlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_pendaftar\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pendaftar',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fpendaftarlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($pendaftar_list)) $pendaftar_list = new cpendaftar_list();

// Page init
$pendaftar_list->Page_Init();

// Page main
$pendaftar_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pendaftar_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($pendaftar->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fpendaftarlist = new ew_Form("fpendaftarlist", "list");
fpendaftarlist.FormKeyCountName = '<?php echo $pendaftar_list->FormKeyCountName ?>';

// Form_CustomValidate event
fpendaftarlist.Form_CustomValidate = 
...
fpendaftarlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fpendaftarlist.Lists["x_nama_ruang"] = {"LinkField":"x_nama_ruang","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_ruang","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"ruang"};
fpendaftarlist.Lists["x_nama_ruang"].Data = "<?php echo $pendaftar_list->nama_ruang->LookupFilterQuery(FALSE, "list") ?>";
fpendaftarlist.Lists["x_nomor_pendaftaran"] = {"LinkField":"x_nomor_pendaftaran","Ajax":true,"AutoFill":false,"DisplayFields":["x_nomor_pendaftaran","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"no_pendaftaran"};
fpendaftarlist.Lists["x_nomor_pendaftaran"].Data = "<?php echo $pendaftar_list->nomor_pendaftaran->LookupFilterQuery(FALSE, "list") ?>";
fpendaftarlist.Lists["x_nomor_peserta_ujian_sdmi"] = {"LinkField":"x_nopes","Ajax":true,"AutoFill":false,"DisplayFields":["x_nopes","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"db_pd"};
fpendaftarlist.Lists["x_nomor_peserta_ujian_sdmi"].Data = "<?php echo $pendaftar_list->nomor_peserta_ujian_sdmi->LookupFilterQuery(FALSE, "list") ?>";
fpendaftarlist.Lists["x_jenis_kelamin"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpendaftarlist.Lists["x_jenis_kelamin"].Options = <?php echo json_encode($pendaftar_list->jenis_kelamin->Options()) ?>;
fpendaftarlist.Lists["x_persyaratan"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpendaftarlist.Lists["x_persyaratan"].Options = <?php echo json_encode($pendaftar_list->persyaratan->Options()) ?>;

// Form object for search
var CurrentSearchForm = fpendaftarlistsrch = new ew_Form("fpendaftarlistsrch");
</script>
<script type="text/javascript">
...
<?php if ($pendaftar->Export == "") { ?>
<div class="ewToolbar">
<?php if ($pendaftar_list->TotalRecs > 0 && $pendaftar_list->ExportOptions->Visible()) { ?>
<?php $pendaftar_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($pendaftar_list->SearchOptions->Visible()) { ?>
<?php $pendaftar_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($pendaftar_list->FilterOptions->Visible()) { ?>
<?php $pendaftar_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $pendaftar_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($pendaftar_list->TotalRecs <= 0)
			$pendaftar_list->TotalRecs = $pendaftar->ListRecordCount();
	} else {
		if (!$pendaftar_list->Recordset && ($pendaftar_list->Recordset = $pendaftar_list->LoadRecordset()))
			$pendaftar_list->TotalRecs = $pendaftar_list->Recordset->RecordCount();
	}
	$pendaftar_list->StartRec = 1;
	if ($pendaftar_list->DisplayRecs <= 0 || ($pendaftar->Export <> "" && $pendaftar->ExportAll)) // Display all records
		$pendaftar_list->DisplayRecs = $pendaftar_list->TotalRecs;
	if (!($pendaftar->Export <> "" && $pendaftar->ExportAll))
		$pendaftar_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$pendaftar_list->Recordset = $pendaftar_list->LoadRecordset($pendaftar_list->StartRec-1, $pendaftar_list->DisplayRecs);

	// Set no record found message
	if ($pendaftar->CurrentAction == "" && $pendaftar_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$pendaftar_list->setWarningMessage(ew_DeniedMsg());
		if ($pendaftar_list->SearchWhere == "0=101")
			$pendaftar_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$pendaftar_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$pendaftar_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($pendaftar->Export == "" && $pendaftar->CurrentAction == "") { ?>
<form name="fpendaftarlistsrch" id="fpendaftarlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($pendaftar_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fpendaftarlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="pendaftar">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($pendaftar_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($pendaftar_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $pendaftar_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($pendaftar_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($pendaftar_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($pendaftar_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($pendaftar_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $pendaftar_list->ShowPageHeader(); ?>
<?php
$pendaftar_list->ShowMessage();
?>
<?php if ($pendaftar_list->TotalRecs > 0 || $pendaftar->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($pendaftar_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> pendaftar">
<?php if ($pendaftar->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($pendaftar->CurrentAction <> "gridadd" && $pendaftar->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($pendaftar_list->Pager)) $pendaftar_list->Pager = new cPrevNextPager($pendaftar_list->StartRec, $pendaftar_list->DisplayRecs, $pendaftar_list->TotalRecs, $pendaftar_list->AutoHidePager) ?>
<?php if ($pendaftar_list->Pager->RecordCount > 0 && $pendaftar_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($pendaftar_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $pendaftar_list->PageUrl() ?>start=<?php echo $pendaftar_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($pendaftar_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $pendaftar_list->PageUrl() ?>start=<?php echo $pendaftar_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $pendaftar_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($pendaftar_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $pendaftar_list->PageUrl() ?>start=<?php echo $pendaftar_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($pendaftar_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $pendaftar_list->PageUrl() ?>start=<?php echo $pendaftar_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $pendaftar_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $pendaftar_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $pendaftar_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $pendaftar_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($pendaftar_list->TotalRecs > 0 && (!$pendaftar_list->AutoHidePageSizeSelector || $pendaftar_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="pendaftar">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($pendaftar_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($pendaftar_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($pendaftar_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($pendaftar_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="ALL"<?php if ($pendaftar->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($pendaftar_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fpendaftarlist" id="fpendaftarlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pendaftar_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pendaftar_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pendaftar">
<div id="gmp_pendaftar" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($pendaftar_list->TotalRecs > 0 || $pendaftar->CurrentAction == "gridedit") { ?>
<table id="tbl_pendaftarlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$pendaftar_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$pendaftar_list->RenderListOptions();

// Render list options (header, left)
$pendaftar_list->ListOptions->Render("header", "left");
?>
<?php if ($pendaftar->nama_ruang->Visible) { // nama_ruang ?>
	<?php if ($pendaftar->SortUrl($pendaftar->nama_ruang) == "") { ?>
		<th data-name="nama_ruang" class="<?php echo $pendaftar->nama_ruang->HeaderCellClass() ?>"><div id="elh_pendaftar_nama_ruang" class="pendaftar_nama_ruang"><div class="ewTableHeaderCaption"><?php echo $pendaftar->nama_ruang->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nama_ruang" class="<?php echo $pendaftar->nama_ruang->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->nama_ruang) ?>',1);"><div id="elh_pendaftar_nama_ruang" class="pendaftar_nama_ruang">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->nama_ruang->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->nama_ruang->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->nama_ruang->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
	<?php if ($pendaftar->SortUrl($pendaftar->nomor_pendaftaran) == "") { ?>
		<th data-name="nomor_pendaftaran" class="<?php echo $pendaftar->nomor_pendaftaran->HeaderCellClass() ?>"><div id="elh_pendaftar_nomor_pendaftaran" class="pendaftar_nomor_pendaftaran"><div class="ewTableHeaderCaption"><?php echo $pendaftar->nomor_pendaftaran->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nomor_pendaftaran" class="<?php echo $pendaftar->nomor_pendaftaran->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->nomor_pendaftaran) ?>',1);"><div id="elh_pendaftar_nomor_pendaftaran" class="pendaftar_nomor_pendaftaran">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->nomor_pendaftaran->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->nomor_pendaftaran->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->nomor_pendaftaran->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
	<?php if ($pendaftar->SortUrl($pendaftar->nomor_peserta_ujian_sdmi) == "") { ?>
		<th data-name="nomor_peserta_ujian_sdmi" class="<?php echo $pendaftar->nomor_peserta_ujian_sdmi->HeaderCellClass() ?>"><div id="elh_pendaftar_nomor_peserta_ujian_sdmi" class="pendaftar_nomor_peserta_ujian_sdmi"><div class="ewTableHeaderCaption"><?php echo $pendaftar->nomor_peserta_ujian_sdmi->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nomor_peserta_ujian_sdmi" class="<?php echo $pendaftar->nomor_peserta_ujian_sdmi->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->nomor_peserta_ujian_sdmi) ?>',1);"><div id="elh_pendaftar_nomor_peserta_ujian_sdmi" class="pendaftar_nomor_peserta_ujian_sdmi">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->nomor_peserta_ujian_sdmi->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->nomor_peserta_ujian_sdmi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->nomor_peserta_ujian_sdmi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->sekolah_asal->Visible) { // sekolah_asal ?>
	<?php if ($pendaftar->SortUrl($pendaftar->sekolah_asal) == "") { ?>
		<th data-name="sekolah_asal" class="<?php echo $pendaftar->sekolah_asal->HeaderCellClass() ?>"><div id="elh_pendaftar_sekolah_asal" class="pendaftar_sekolah_asal"><div class="ewTableHeaderCaption"><?php echo $pendaftar->sekolah_asal->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sekolah_asal" class="<?php echo $pendaftar->sekolah_asal->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->sekolah_asal) ?>',1);"><div id="elh_pendaftar_sekolah_asal" class="pendaftar_sekolah_asal">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->sekolah_asal->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->sekolah_asal->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->sekolah_asal->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->nama_lengkap->Visible) { // nama_lengkap ?>
	<?php if ($pendaftar->SortUrl($pendaftar->nama_lengkap) == "") { ?>
		<th data-name="nama_lengkap" class="<?php echo $pendaftar->nama_lengkap->HeaderCellClass() ?>"><div id="elh_pendaftar_nama_lengkap" class="pendaftar_nama_lengkap"><div class="ewTableHeaderCaption"><?php echo $pendaftar->nama_lengkap->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nama_lengkap" class="<?php echo $pendaftar->nama_lengkap->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->nama_lengkap) ?>',1);"><div id="elh_pendaftar_nama_lengkap" class="pendaftar_nama_lengkap">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->nama_lengkap->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->nama_lengkap->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->nama_lengkap->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->jenis_kelamin->Visible) { // jenis_kelamin ?>
	<?php if ($pendaftar->SortUrl($pendaftar->jenis_kelamin) == "") { ?>
		<th data-name="jenis_kelamin" class="<?php echo $pendaftar->jenis_kelamin->HeaderCellClass() ?>"><div id="elh_pendaftar_jenis_kelamin" class="pendaftar_jenis_kelamin"><div class="ewTableHeaderCaption"><?php echo $pendaftar->jenis_kelamin->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="jenis_kelamin" class="<?php echo $pendaftar->jenis_kelamin->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->jenis_kelamin) ?>',1);"><div id="elh_pendaftar_jenis_kelamin" class="pendaftar_jenis_kelamin">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->jenis_kelamin->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->jenis_kelamin->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->jenis_kelamin->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->zona->Visible) { // zona ?>
	<?php if ($pendaftar->SortUrl($pendaftar->zona) == "") { ?>
		<th data-name="zona" class="<?php echo $pendaftar->zona->HeaderCellClass() ?>"><div id="elh_pendaftar_zona" class="pendaftar_zona"><div class="ewTableHeaderCaption"><?php echo $pendaftar->zona->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="zona" class="<?php echo $pendaftar->zona->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->zona) ?>',1);"><div id="elh_pendaftar_zona" class="pendaftar_zona">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->zona->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->zona->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->zona->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->nilai_akhir->Visible) { // nilai_akhir ?>
	<?php if ($pendaftar->SortUrl($pendaftar->nilai_akhir) == "") { ?>
		<th data-name="nilai_akhir" class="<?php echo $pendaftar->nilai_akhir->HeaderCellClass() ?>"><div id="elh_pendaftar_nilai_akhir" class="pendaftar_nilai_akhir"><div class="ewTableHeaderCaption"><?php echo $pendaftar->nilai_akhir->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nilai_akhir" class="<?php echo $pendaftar->nilai_akhir->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->nilai_akhir) ?>',1);"><div id="elh_pendaftar_nilai_akhir" class="pendaftar_nilai_akhir">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->nilai_akhir->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->nilai_akhir->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->nilai_akhir->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->status->Visible) { // status ?>
	<?php if ($pendaftar->SortUrl($pendaftar->status) == "") { ?>
		<th data-name="status" class="<?php echo $pendaftar->status->HeaderCellClass() ?>"><div id="elh_pendaftar_status" class="pendaftar_status"><div class="ewTableHeaderCaption"><?php echo $pendaftar->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $pendaftar->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->status) ?>',1);"><div id="elh_pendaftar_status" class="pendaftar_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->status->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->persyaratan->Visible) { // persyaratan ?>
	<?php if ($pendaftar->SortUrl($pendaftar->persyaratan) == "") { ?>
		<th data-name="persyaratan" class="<?php echo $pendaftar->persyaratan->HeaderCellClass() ?>"><div id="elh_pendaftar_persyaratan" class="pendaftar_persyaratan"><div class="ewTableHeaderCaption"><?php echo $pendaftar->persyaratan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="persyaratan" class="<?php echo $pendaftar->persyaratan->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->persyaratan) ?>',1);"><div id="elh_pendaftar_persyaratan" class="pendaftar_persyaratan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->persyaratan->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->persyaratan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->persyaratan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pendaftar->catatan->Visible) { // catatan ?>
	<?php if ($pendaftar->SortUrl($pendaftar->catatan) == "") { ?>
		<th data-name="catatan" class="<?php echo $pendaftar->catatan->HeaderCellClass() ?>"><div id="elh_pendaftar_catatan" class="pendaftar_catatan"><div class="ewTableHeaderCaption"><?php echo $pendaftar->catatan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="catatan" class="<?php echo $pendaftar->catatan->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pendaftar->SortUrl($pendaftar->catatan) ?>',1);"><div id="elh_pendaftar_catatan" class="pendaftar_catatan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pendaftar->catatan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pendaftar->catatan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pendaftar->catatan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$pendaftar_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($pendaftar->ExportAll && $pendaftar->Export <> "") {
	$pendaftar_list->StopRec = $pendaftar_list->TotalRecs;
} else {

	// Set the last record to display
	if ($pendaftar_list->TotalRecs > $pendaftar_list->StartRec + $pendaftar_list->DisplayRecs - 1)
		$pendaftar_list->StopRec = $pendaftar_list->StartRec + $pendaftar_list->DisplayRecs - 1;
	else
		$pendaftar_list->StopRec = $pendaftar_list->TotalRecs;
}
$pendaftar_list->RecCnt = $pendaftar_list->StartRec - 1;
if ($pendaftar_list->Recordset && !$pendaftar_list->Recordset->EOF) {
	$pendaftar_list->Recordset->MoveFirst();
	$bSelectLimit = $pendaftar_list->UseSelectLimit;
	if (!$bSelectLimit && $pendaftar_list->StartRec > 1)
		$pendaftar_list->Recordset->Move($pendaftar_list->StartRec - 1);
} elseif (!$pendaftar->AllowAddDeleteRow && $pendaftar_list->StopRec == 0) {
	$pendaftar_list->StopRec = $pendaftar->GridAddRowCount;
}

// Initialize aggregate
$pendaftar->RowType = EW_ROWTYPE_AGGREGATEINIT;
$pendaftar->ResetAttrs();
$pendaftar_list->RenderRow();
while ($pendaftar_list->RecCnt < $pendaftar_list->StopRec) {
	$pendaftar_list->RecCnt++;
	if (intval($pendaftar_list->RecCnt) >= intval($pendaftar_list->StartRec)) {
		$pendaftar_list->RowCnt++;

		// Set up key count
		$pendaftar_list->KeyCount = $pendaftar_list->RowIndex;

		// Init row class and style
		$pendaftar->ResetAttrs();
		$pendaftar->CssClass = "";
		if ($pendaftar->CurrentAction == "gridadd") {
		} else {
			$pendaftar_list->LoadRowValues($pendaftar_list->Recordset); // Load row values
		}
		$pendaftar->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$pendaftar->RowAttrs = array_merge($pendaftar->RowAttrs, array('data-rowindex'=>$pendaftar_list->RowCnt, 'id'=>'r' . $pendaftar_list->RowCnt . '_pendaftar', 'data-rowtype'=>$pendaftar->RowType));
...
$pendaftar_list->ListOptions->Render("body", "left", $pendaftar_list->RowCnt);
?>
	<?php if ($pendaftar->nama_ruang->Visible) { // nama_ruang ?>
		<td data-name="nama_ruang"<?php echo $pendaftar->nama_ruang->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_nama_ruang" class="pendaftar_nama_ruang">
...
<?php echo $pendaftar->nama_ruang->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
		<td data-name="nomor_pendaftaran"<?php echo $pendaftar->nomor_pendaftaran->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_nomor_pendaftaran" class="pendaftar_nomor_pendaftaran">
...
<?php echo $pendaftar->nomor_pendaftaran->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
		<td data-name="nomor_peserta_ujian_sdmi"<?php echo $pendaftar->nomor_peserta_ujian_sdmi->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_nomor_peserta_ujian_sdmi" class="pendaftar_nomor_peserta_ujian_sdmi">
...
<?php echo $pendaftar->nomor_peserta_ujian_sdmi->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->sekolah_asal->Visible) { // sekolah_asal ?>
		<td data-name="sekolah_asal"<?php echo $pendaftar->sekolah_asal->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_sekolah_asal" class="pendaftar_sekolah_asal">
...
<?php echo $pendaftar->sekolah_asal->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->nama_lengkap->Visible) { // nama_lengkap ?>
		<td data-name="nama_lengkap"<?php echo $pendaftar->nama_lengkap->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_nama_lengkap" class="pendaftar_nama_lengkap">
...
<?php echo $pendaftar->nama_lengkap->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->jenis_kelamin->Visible) { // jenis_kelamin ?>
		<td data-name="jenis_kelamin"<?php echo $pendaftar->jenis_kelamin->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_jenis_kelamin" class="pendaftar_jenis_kelamin">
...
<?php echo $pendaftar->jenis_kelamin->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->zona->Visible) { // zona ?>
		<td data-name="zona"<?php echo $pendaftar->zona->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_zona" class="pendaftar_zona">
...
<?php echo $pendaftar->zona->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->nilai_akhir->Visible) { // nilai_akhir ?>
		<td data-name="nilai_akhir"<?php echo $pendaftar->nilai_akhir->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_nilai_akhir" class="pendaftar_nilai_akhir">
...
<?php echo $pendaftar->nilai_akhir->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->status->Visible) { // status ?>
		<td data-name="status"<?php echo $pendaftar->status->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_status" class="pendaftar_status">
...
<?php echo $pendaftar->status->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->persyaratan->Visible) { // persyaratan ?>
		<td data-name="persyaratan"<?php echo $pendaftar->persyaratan->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_persyaratan" class="pendaftar_persyaratan">
...
<?php echo $pendaftar->persyaratan->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($pendaftar->catatan->Visible) { // catatan ?>
		<td data-name="catatan"<?php echo $pendaftar->catatan->CellAttributes() ?>>
<span id="el<?php echo $pendaftar_list->RowCnt ?>_pendaftar_catatan" class="pendaftar_catatan">
...
$pendaftar_list->ListOptions->Render("body", "right", $pendaftar_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($pendaftar->CurrentAction <> "gridadd")
		$pendaftar_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$pendaftar->RowType = EW_ROWTYPE_AGGREGATE;
$pendaftar->ResetAttrs();
$pendaftar_list->RenderRow();
?>
<?php if ($pendaftar_list->TotalRecs > 0 && ($pendaftar->CurrentAction <> "gridadd" && $pendaftar->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$pendaftar_list->ListOptions->Render("footer", "left");
?>
	<?php if ($pendaftar->nama_ruang->Visible) { // nama_ruang ?>
		<td data-name="nama_ruang" class="<?php echo $pendaftar->nama_ruang->FooterCellClass() ?>"><span id="elf_pendaftar_nama_ruang" class="pendaftar_nama_ruang">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
		<td data-name="nomor_pendaftaran" class="<?php echo $pendaftar->nomor_pendaftaran->FooterCellClass() ?>"><span id="elf_pendaftar_nomor_pendaftaran" class="pendaftar_nomor_pendaftaran">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
		<td data-name="nomor_peserta_ujian_sdmi" class="<?php echo $pendaftar->nomor_peserta_ujian_sdmi->FooterCellClass() ?>"><span id="elf_pendaftar_nomor_peserta_ujian_sdmi" class="pendaftar_nomor_peserta_ujian_sdmi">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->sekolah_asal->Visible) { // sekolah_asal ?>
		<td data-name="sekolah_asal" class="<?php echo $pendaftar->sekolah_asal->FooterCellClass() ?>"><span id="elf_pendaftar_sekolah_asal" class="pendaftar_sekolah_asal">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->nama_lengkap->Visible) { // nama_lengkap ?>
		<td data-name="nama_lengkap" class="<?php echo $pendaftar->nama_lengkap->FooterCellClass() ?>"><span id="elf_pendaftar_nama_lengkap" class="pendaftar_nama_lengkap">
<span class="ewAggregate"><?php echo $Language->Phrase("COUNT") ?></span><span class="ewAggregateValue">
<?php echo $pendaftar->nama_lengkap->ViewValue ?></span>
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->jenis_kelamin->Visible) { // jenis_kelamin ?>
		<td data-name="jenis_kelamin" class="<?php echo $pendaftar->jenis_kelamin->FooterCellClass() ?>"><span id="elf_pendaftar_jenis_kelamin" class="pendaftar_jenis_kelamin">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->zona->Visible) { // zona ?>
		<td data-name="zona" class="<?php echo $pendaftar->zona->FooterCellClass() ?>"><span id="elf_pendaftar_zona" class="pendaftar_zona">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->nilai_akhir->Visible) { // nilai_akhir ?>
		<td data-name="nilai_akhir" class="<?php echo $pendaftar->nilai_akhir->FooterCellClass() ?>"><span id="elf_pendaftar_nilai_akhir" class="pendaftar_nilai_akhir">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->status->Visible) { // status ?>
		<td data-name="status" class="<?php echo $pendaftar->status->FooterCellClass() ?>"><span id="elf_pendaftar_status" class="pendaftar_status">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->persyaratan->Visible) { // persyaratan ?>
		<td data-name="persyaratan" class="<?php echo $pendaftar->persyaratan->FooterCellClass() ?>"><span id="elf_pendaftar_persyaratan" class="pendaftar_persyaratan">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($pendaftar->catatan->Visible) { // catatan ?>
		<td data-name="catatan" class="<?php echo $pendaftar->catatan->FooterCellClass() ?>"><span id="elf_pendaftar_catatan" class="pendaftar_catatan">
		&nbsp;
		</span></td>
	<?php } ?>
<?php
...
$pendaftar_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>
<?php } ?>
...
<?php } ?>
<?php if ($pendaftar->CurrentAction == "") { ?>
...
// Close recordset
if ($pendaftar_list->Recordset)
	$pendaftar_list->Recordset->Close();
?>
<?php if ($pendaftar->Export == "") { ?>
...
<?php } ?>
<?php if ($pendaftar_list->TotalRecs == 0 && $pendaftar->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($pendaftar_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($pendaftar->Export == "") { ?>
<script type="text/javascript">
fpendaftarlistsrch.FilterList = <?php echo $pendaftar_list->GetFilterList() ?>;
fpendaftarlistsrch.Init();
fpendaftarlist.Init();
</script>
<?php } ?>
<?php
$pendaftar_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($pendaftar->Export == "") { ?>

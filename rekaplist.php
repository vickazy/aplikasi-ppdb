...
<?php include_once "rekapinfo.php" ?>
...
$rekap_list = NULL; // Initialize page object first

class crekap_list extends crekap {
	...
	// Page object name
	var $PageObjName = 'rekap_list';

	// Grid form hidden field names
	var $FormName = 'frekaplist';
	...
	function __construct() {
		...
		// Table object (rekap)
		if (!isset($GLOBALS["rekap"]) || get_class($GLOBALS["rekap"]) == "crekap") {
			$GLOBALS["rekap"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["rekap"];
		}
		...
		$this->AddUrl = "rekapadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "rekapdelete.php";
		$this->MultiUpdateUrl = "rekapupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'rekap', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption frekaplistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "frekaplistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "frekaplistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"frekaplistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"frekaplistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.frekaplist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"frekaplistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_rekap\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_rekap',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.frekaplist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($rekap_list)) $rekap_list = new crekap_list();

// Page init
$rekap_list->Page_Init();

// Page main
$rekap_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$rekap_list->Page_Render();
?>
...
var CurrentForm = frekaplist = new ew_Form("frekaplist", "list");
frekaplist.FormKeyCountName = '<?php echo $rekap_list->FormKeyCountName ?>';

// Form_CustomValidate event
frekaplist.Form_CustomValidate = 
...
frekaplist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
frekaplist.Lists["x_nama_ruang"] = {"LinkField":"x_nama_ruang","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_ruang","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"ruang"};
frekaplist.Lists["x_nama_ruang"].Data = "<?php echo $rekap_list->nama_ruang->LookupFilterQuery(FALSE, "list") ?>";
frekaplist.AutoSuggests["x_nama_ruang"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $rekap_list->nama_ruang->LookupFilterQuery(TRUE, "list"))) ?>;
frekaplist.Lists["x_nomor_pendaftaran"] = {"LinkField":"x_nomor_pendaftaran","Ajax":true,"AutoFill":false,"DisplayFields":["x_nomor_pendaftaran","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"no_pendaftaran"};
frekaplist.Lists["x_nomor_pendaftaran"].Data = "<?php echo $rekap_list->nomor_pendaftaran->LookupFilterQuery(FALSE, "list") ?>";
frekaplist.AutoSuggests["x_nomor_pendaftaran"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $rekap_list->nomor_pendaftaran->LookupFilterQuery(TRUE, "list"))) ?>;
frekaplist.Lists["x_nomor_peserta_ujian_sdmi"] = {"LinkField":"x_nopes","Ajax":true,"AutoFill":false,"DisplayFields":["x_nopes","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"db_pd"};
frekaplist.Lists["x_nomor_peserta_ujian_sdmi"].Data = "<?php echo $rekap_list->nomor_peserta_ujian_sdmi->LookupFilterQuery(FALSE, "list") ?>";
frekaplist.AutoSuggests["x_nomor_peserta_ujian_sdmi"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $rekap_list->nomor_peserta_ujian_sdmi->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = frekaplistsrch = new ew_Form("frekaplistsrch");
</script>
<script type="text/javascript">
...
<?php if ($rekap->Export == "") { ?>
<div class="ewToolbar">
<?php if ($rekap_list->TotalRecs > 0 && $rekap_list->ExportOptions->Visible()) { ?>
<?php $rekap_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($rekap_list->SearchOptions->Visible()) { ?>
<?php $rekap_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($rekap_list->FilterOptions->Visible()) { ?>
<?php $rekap_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $rekap_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($rekap_list->TotalRecs <= 0)
			$rekap_list->TotalRecs = $rekap->ListRecordCount();
	} else {
		if (!$rekap_list->Recordset && ($rekap_list->Recordset = $rekap_list->LoadRecordset()))
			$rekap_list->TotalRecs = $rekap_list->Recordset->RecordCount();
	}
	$rekap_list->StartRec = 1;
	if ($rekap_list->DisplayRecs <= 0 || ($rekap->Export <> "" && $rekap->ExportAll)) // Display all records
		$rekap_list->DisplayRecs = $rekap_list->TotalRecs;
	if (!($rekap->Export <> "" && $rekap->ExportAll))
		$rekap_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$rekap_list->Recordset = $rekap_list->LoadRecordset($rekap_list->StartRec-1, $rekap_list->DisplayRecs);

	// Set no record found message
	if ($rekap->CurrentAction == "" && $rekap_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$rekap_list->setWarningMessage(ew_DeniedMsg());
		if ($rekap_list->SearchWhere == "0=101")
			$rekap_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$rekap_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$rekap_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($rekap->Export == "" && $rekap->CurrentAction == "") { ?>
<form name="frekaplistsrch" id="frekaplistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($rekap_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="frekaplistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="rekap">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($rekap_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($rekap_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $rekap_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($rekap_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($rekap_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($rekap_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($rekap_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $rekap_list->ShowPageHeader(); ?>
<?php
$rekap_list->ShowMessage();
?>
<?php if ($rekap_list->TotalRecs > 0 || $rekap->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($rekap_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> rekap">
<?php if ($rekap->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($rekap->CurrentAction <> "gridadd" && $rekap->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($rekap_list->Pager)) $rekap_list->Pager = new cPrevNextPager($rekap_list->StartRec, $rekap_list->DisplayRecs, $rekap_list->TotalRecs, $rekap_list->AutoHidePager) ?>
<?php if ($rekap_list->Pager->RecordCount > 0 && $rekap_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($rekap_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $rekap_list->PageUrl() ?>start=<?php echo $rekap_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($rekap_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $rekap_list->PageUrl() ?>start=<?php echo $rekap_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $rekap_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($rekap_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $rekap_list->PageUrl() ?>start=<?php echo $rekap_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($rekap_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $rekap_list->PageUrl() ?>start=<?php echo $rekap_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $rekap_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $rekap_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $rekap_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $rekap_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($rekap_list->TotalRecs > 0 && (!$rekap_list->AutoHidePageSizeSelector || $rekap_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="rekap">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($rekap_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($rekap_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($rekap_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($rekap_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="ALL"<?php if ($rekap->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($rekap_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="frekaplist" id="frekaplist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($rekap_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $rekap_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="rekap">
<div id="gmp_rekap" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($rekap_list->TotalRecs > 0 || $rekap->CurrentAction == "gridedit") { ?>
<table id="tbl_rekaplist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$rekap_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$rekap_list->RenderListOptions();

// Render list options (header, left)
$rekap_list->ListOptions->Render("header", "left");
?>
<?php if ($rekap->nama_ruang->Visible) { // nama_ruang ?>
	<?php if ($rekap->SortUrl($rekap->nama_ruang) == "") { ?>
		<th data-name="nama_ruang" class="<?php echo $rekap->nama_ruang->HeaderCellClass() ?>"><div id="elh_rekap_nama_ruang" class="rekap_nama_ruang"><div class="ewTableHeaderCaption"><?php echo $rekap->nama_ruang->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nama_ruang" class="<?php echo $rekap->nama_ruang->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->nama_ruang) ?>',1);"><div id="elh_rekap_nama_ruang" class="rekap_nama_ruang">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->nama_ruang->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->nama_ruang->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->nama_ruang->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
	<?php if ($rekap->SortUrl($rekap->nomor_pendaftaran) == "") { ?>
		<th data-name="nomor_pendaftaran" class="<?php echo $rekap->nomor_pendaftaran->HeaderCellClass() ?>"><div id="elh_rekap_nomor_pendaftaran" class="rekap_nomor_pendaftaran"><div class="ewTableHeaderCaption"><?php echo $rekap->nomor_pendaftaran->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nomor_pendaftaran" class="<?php echo $rekap->nomor_pendaftaran->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->nomor_pendaftaran) ?>',1);"><div id="elh_rekap_nomor_pendaftaran" class="rekap_nomor_pendaftaran">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->nomor_pendaftaran->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->nomor_pendaftaran->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->nomor_pendaftaran->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
	<?php if ($rekap->SortUrl($rekap->nomor_peserta_ujian_sdmi) == "") { ?>
		<th data-name="nomor_peserta_ujian_sdmi" class="<?php echo $rekap->nomor_peserta_ujian_sdmi->HeaderCellClass() ?>"><div id="elh_rekap_nomor_peserta_ujian_sdmi" class="rekap_nomor_peserta_ujian_sdmi"><div class="ewTableHeaderCaption"><?php echo $rekap->nomor_peserta_ujian_sdmi->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nomor_peserta_ujian_sdmi" class="<?php echo $rekap->nomor_peserta_ujian_sdmi->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->nomor_peserta_ujian_sdmi) ?>',1);"><div id="elh_rekap_nomor_peserta_ujian_sdmi" class="rekap_nomor_peserta_ujian_sdmi">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->nomor_peserta_ujian_sdmi->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->nomor_peserta_ujian_sdmi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->nomor_peserta_ujian_sdmi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->sekolah_asal->Visible) { // sekolah_asal ?>
	<?php if ($rekap->SortUrl($rekap->sekolah_asal) == "") { ?>
		<th data-name="sekolah_asal" class="<?php echo $rekap->sekolah_asal->HeaderCellClass() ?>"><div id="elh_rekap_sekolah_asal" class="rekap_sekolah_asal"><div class="ewTableHeaderCaption"><?php echo $rekap->sekolah_asal->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sekolah_asal" class="<?php echo $rekap->sekolah_asal->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->sekolah_asal) ?>',1);"><div id="elh_rekap_sekolah_asal" class="rekap_sekolah_asal">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->sekolah_asal->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->sekolah_asal->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->sekolah_asal->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->nama_lengkap->Visible) { // nama_lengkap ?>
	<?php if ($rekap->SortUrl($rekap->nama_lengkap) == "") { ?>
		<th data-name="nama_lengkap" class="<?php echo $rekap->nama_lengkap->HeaderCellClass() ?>"><div id="elh_rekap_nama_lengkap" class="rekap_nama_lengkap"><div class="ewTableHeaderCaption"><?php echo $rekap->nama_lengkap->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nama_lengkap" class="<?php echo $rekap->nama_lengkap->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->nama_lengkap) ?>',1);"><div id="elh_rekap_nama_lengkap" class="rekap_nama_lengkap">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->nama_lengkap->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->nama_lengkap->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->nama_lengkap->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->jenis_kelamin->Visible) { // jenis_kelamin ?>
	<?php if ($rekap->SortUrl($rekap->jenis_kelamin) == "") { ?>
		<th data-name="jenis_kelamin" class="<?php echo $rekap->jenis_kelamin->HeaderCellClass() ?>"><div id="elh_rekap_jenis_kelamin" class="rekap_jenis_kelamin"><div class="ewTableHeaderCaption"><?php echo $rekap->jenis_kelamin->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="jenis_kelamin" class="<?php echo $rekap->jenis_kelamin->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->jenis_kelamin) ?>',1);"><div id="elh_rekap_jenis_kelamin" class="rekap_jenis_kelamin">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->jenis_kelamin->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->jenis_kelamin->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->jenis_kelamin->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->zona->Visible) { // zona ?>
	<?php if ($rekap->SortUrl($rekap->zona) == "") { ?>
		<th data-name="zona" class="<?php echo $rekap->zona->HeaderCellClass() ?>"><div id="elh_rekap_zona" class="rekap_zona"><div class="ewTableHeaderCaption"><?php echo $rekap->zona->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="zona" class="<?php echo $rekap->zona->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->zona) ?>',1);"><div id="elh_rekap_zona" class="rekap_zona">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->zona->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->zona->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->zona->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->nilai_akhir->Visible) { // nilai_akhir ?>
	<?php if ($rekap->SortUrl($rekap->nilai_akhir) == "") { ?>
		<th data-name="nilai_akhir" class="<?php echo $rekap->nilai_akhir->HeaderCellClass() ?>"><div id="elh_rekap_nilai_akhir" class="rekap_nilai_akhir"><div class="ewTableHeaderCaption"><?php echo $rekap->nilai_akhir->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nilai_akhir" class="<?php echo $rekap->nilai_akhir->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->nilai_akhir) ?>',1);"><div id="elh_rekap_nilai_akhir" class="rekap_nilai_akhir">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->nilai_akhir->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($rekap->nilai_akhir->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->nilai_akhir->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->status->Visible) { // status ?>
	<?php if ($rekap->SortUrl($rekap->status) == "") { ?>
		<th data-name="status" class="<?php echo $rekap->status->HeaderCellClass() ?>"><div id="elh_rekap_status" class="rekap_status"><div class="ewTableHeaderCaption"><?php echo $rekap->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $rekap->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->status) ?>',1);"><div id="elh_rekap_status" class="rekap_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->status->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->persyaratan->Visible) { // persyaratan ?>
	<?php if ($rekap->SortUrl($rekap->persyaratan) == "") { ?>
		<th data-name="persyaratan" class="<?php echo $rekap->persyaratan->HeaderCellClass() ?>"><div id="elh_rekap_persyaratan" class="rekap_persyaratan"><div class="ewTableHeaderCaption"><?php echo $rekap->persyaratan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="persyaratan" class="<?php echo $rekap->persyaratan->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->persyaratan) ?>',1);"><div id="elh_rekap_persyaratan" class="rekap_persyaratan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->persyaratan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->persyaratan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->persyaratan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($rekap->catatan->Visible) { // catatan ?>
	<?php if ($rekap->SortUrl($rekap->catatan) == "") { ?>
		<th data-name="catatan" class="<?php echo $rekap->catatan->HeaderCellClass() ?>"><div id="elh_rekap_catatan" class="rekap_catatan"><div class="ewTableHeaderCaption"><?php echo $rekap->catatan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="catatan" class="<?php echo $rekap->catatan->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $rekap->SortUrl($rekap->catatan) ?>',1);"><div id="elh_rekap_catatan" class="rekap_catatan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $rekap->catatan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($rekap->catatan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($rekap->catatan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$rekap_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($rekap->ExportAll && $rekap->Export <> "") {
	$rekap_list->StopRec = $rekap_list->TotalRecs;
} else {

	// Set the last record to display
	if ($rekap_list->TotalRecs > $rekap_list->StartRec + $rekap_list->DisplayRecs - 1)
		$rekap_list->StopRec = $rekap_list->StartRec + $rekap_list->DisplayRecs - 1;
	else
		$rekap_list->StopRec = $rekap_list->TotalRecs;
}
$rekap_list->RecCnt = $rekap_list->StartRec - 1;
if ($rekap_list->Recordset && !$rekap_list->Recordset->EOF) {
	$rekap_list->Recordset->MoveFirst();
	$bSelectLimit = $rekap_list->UseSelectLimit;
	if (!$bSelectLimit && $rekap_list->StartRec > 1)
		$rekap_list->Recordset->Move($rekap_list->StartRec - 1);
} elseif (!$rekap->AllowAddDeleteRow && $rekap_list->StopRec == 0) {
	$rekap_list->StopRec = $rekap->GridAddRowCount;
}

// Initialize aggregate
$rekap->RowType = EW_ROWTYPE_AGGREGATEINIT;
$rekap->ResetAttrs();
$rekap_list->RenderRow();
while ($rekap_list->RecCnt < $rekap_list->StopRec) {
	$rekap_list->RecCnt++;
	if (intval($rekap_list->RecCnt) >= intval($rekap_list->StartRec)) {
		$rekap_list->RowCnt++;

		// Set up key count
		$rekap_list->KeyCount = $rekap_list->RowIndex;

		// Init row class and style
		$rekap->ResetAttrs();
		$rekap->CssClass = "";
		if ($rekap->CurrentAction == "gridadd") {
		} else {
			$rekap_list->LoadRowValues($rekap_list->Recordset); // Load row values
		}
		$rekap->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$rekap->RowAttrs = array_merge($rekap->RowAttrs, array('data-rowindex'=>$rekap_list->RowCnt, 'id'=>'r' . $rekap_list->RowCnt . '_rekap', 'data-rowtype'=>$rekap->RowType));
		...
		$rekap_list->RenderListOptions();
?>
	<tr<?php echo $rekap->RowAttributes() ?>>
<?php

// Render list options (body, left)
$rekap_list->ListOptions->Render("body", "left", $rekap_list->RowCnt);
?>
	<?php if ($rekap->nama_ruang->Visible) { // nama_ruang ?>
		<td data-name="nama_ruang"<?php echo $rekap->nama_ruang->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_nama_ruang" class="rekap_nama_ruang">
<span<?php echo $rekap->nama_ruang->ViewAttributes() ?>>
<?php echo $rekap->nama_ruang->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
		<td data-name="nomor_pendaftaran"<?php echo $rekap->nomor_pendaftaran->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_nomor_pendaftaran" class="rekap_nomor_pendaftaran">
<span<?php echo $rekap->nomor_pendaftaran->ViewAttributes() ?>>
<?php echo $rekap->nomor_pendaftaran->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
		<td data-name="nomor_peserta_ujian_sdmi"<?php echo $rekap->nomor_peserta_ujian_sdmi->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_nomor_peserta_ujian_sdmi" class="rekap_nomor_peserta_ujian_sdmi">
<span<?php echo $rekap->nomor_peserta_ujian_sdmi->ViewAttributes() ?>>
<?php echo $rekap->nomor_peserta_ujian_sdmi->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->sekolah_asal->Visible) { // sekolah_asal ?>
		<td data-name="sekolah_asal"<?php echo $rekap->sekolah_asal->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_sekolah_asal" class="rekap_sekolah_asal">
<span<?php echo $rekap->sekolah_asal->ViewAttributes() ?>>
<?php echo $rekap->sekolah_asal->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->nama_lengkap->Visible) { // nama_lengkap ?>
		<td data-name="nama_lengkap"<?php echo $rekap->nama_lengkap->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_nama_lengkap" class="rekap_nama_lengkap">
<span<?php echo $rekap->nama_lengkap->ViewAttributes() ?>>
<?php echo $rekap->nama_lengkap->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->jenis_kelamin->Visible) { // jenis_kelamin ?>
		<td data-name="jenis_kelamin"<?php echo $rekap->jenis_kelamin->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_jenis_kelamin" class="rekap_jenis_kelamin">
<span<?php echo $rekap->jenis_kelamin->ViewAttributes() ?>>
<?php echo $rekap->jenis_kelamin->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->zona->Visible) { // zona ?>
		<td data-name="zona"<?php echo $rekap->zona->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_zona" class="rekap_zona">
<span<?php echo $rekap->zona->ViewAttributes() ?>>
<?php echo $rekap->zona->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->nilai_akhir->Visible) { // nilai_akhir ?>
		<td data-name="nilai_akhir"<?php echo $rekap->nilai_akhir->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_nilai_akhir" class="rekap_nilai_akhir">
<span<?php echo $rekap->nilai_akhir->ViewAttributes() ?>>
<?php echo $rekap->nilai_akhir->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->status->Visible) { // status ?>
		<td data-name="status"<?php echo $rekap->status->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_status" class="rekap_status">
<span<?php echo $rekap->status->ViewAttributes() ?>>
<?php echo $rekap->status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->persyaratan->Visible) { // persyaratan ?>
		<td data-name="persyaratan"<?php echo $rekap->persyaratan->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_persyaratan" class="rekap_persyaratan">
<span<?php echo $rekap->persyaratan->ViewAttributes() ?>>
<?php echo $rekap->persyaratan->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($rekap->catatan->Visible) { // catatan ?>
		<td data-name="catatan"<?php echo $rekap->catatan->CellAttributes() ?>>
<span id="el<?php echo $rekap_list->RowCnt ?>_rekap_catatan" class="rekap_catatan">
<span<?php echo $rekap->catatan->ViewAttributes() ?>>
<?php echo $rekap->catatan->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$rekap_list->ListOptions->Render("body", "right", $rekap_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($rekap->CurrentAction <> "gridadd")
		$rekap_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$rekap->RowType = EW_ROWTYPE_AGGREGATE;
$rekap->ResetAttrs();
$rekap_list->RenderRow();
?>
<?php if ($rekap_list->TotalRecs > 0 && ($rekap->CurrentAction <> "gridadd" && $rekap->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$rekap_list->ListOptions->Render("footer", "left");
?>
	<?php if ($rekap->nama_ruang->Visible) { // nama_ruang ?>
		<td data-name="nama_ruang" class="<?php echo $rekap->nama_ruang->FooterCellClass() ?>"><span id="elf_rekap_nama_ruang" class="rekap_nama_ruang">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->nomor_pendaftaran->Visible) { // nomor_pendaftaran ?>
		<td data-name="nomor_pendaftaran" class="<?php echo $rekap->nomor_pendaftaran->FooterCellClass() ?>"><span id="elf_rekap_nomor_pendaftaran" class="rekap_nomor_pendaftaran">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->nomor_peserta_ujian_sdmi->Visible) { // nomor_peserta_ujian_sdmi ?>
		<td data-name="nomor_peserta_ujian_sdmi" class="<?php echo $rekap->nomor_peserta_ujian_sdmi->FooterCellClass() ?>"><span id="elf_rekap_nomor_peserta_ujian_sdmi" class="rekap_nomor_peserta_ujian_sdmi">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->sekolah_asal->Visible) { // sekolah_asal ?>
		<td data-name="sekolah_asal" class="<?php echo $rekap->sekolah_asal->FooterCellClass() ?>"><span id="elf_rekap_sekolah_asal" class="rekap_sekolah_asal">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->nama_lengkap->Visible) { // nama_lengkap ?>
		<td data-name="nama_lengkap" class="<?php echo $rekap->nama_lengkap->FooterCellClass() ?>"><span id="elf_rekap_nama_lengkap" class="rekap_nama_lengkap">
<span class="ewAggregate"><?php echo $Language->Phrase("COUNT") ?></span><span class="ewAggregateValue">
<?php echo $rekap->nama_lengkap->ViewValue ?></span>
		</span></td>
	<?php } ?>
	<?php if ($rekap->jenis_kelamin->Visible) { // jenis_kelamin ?>
		<td data-name="jenis_kelamin" class="<?php echo $rekap->jenis_kelamin->FooterCellClass() ?>"><span id="elf_rekap_jenis_kelamin" class="rekap_jenis_kelamin">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->zona->Visible) { // zona ?>
		<td data-name="zona" class="<?php echo $rekap->zona->FooterCellClass() ?>"><span id="elf_rekap_zona" class="rekap_zona">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->nilai_akhir->Visible) { // nilai_akhir ?>
		<td data-name="nilai_akhir" class="<?php echo $rekap->nilai_akhir->FooterCellClass() ?>"><span id="elf_rekap_nilai_akhir" class="rekap_nilai_akhir">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->status->Visible) { // status ?>
		<td data-name="status" class="<?php echo $rekap->status->FooterCellClass() ?>"><span id="elf_rekap_status" class="rekap_status">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->persyaratan->Visible) { // persyaratan ?>
		<td data-name="persyaratan" class="<?php echo $rekap->persyaratan->FooterCellClass() ?>"><span id="elf_rekap_persyaratan" class="rekap_persyaratan">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($rekap->catatan->Visible) { // catatan ?>
		<td data-name="catatan" class="<?php echo $rekap->catatan->FooterCellClass() ?>"><span id="elf_rekap_catatan" class="rekap_catatan">
		&nbsp;
		</span></td>
	<?php } ?>
<?php
...
$rekap_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>
<?php } ?>
...
// Close recordset
if ($rekap_list->Recordset)
	$rekap_list->Recordset->Close();
?>
<?php if ($rekap->Export == "") { ?>
...
<?php } ?>
<?php if ($rekap_list->TotalRecs == 0 && $rekap->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($rekap_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($rekap->Export == "") { ?>
<script type="text/javascript">
frekaplistsrch.FilterList = <?php echo $rekap_list->GetFilterList() ?>;
frekaplistsrch.Init();
frekaplist.Init();
</script>
<?php } ?>
<?php
$rekap_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($rekap->Export == "") { ?>

...
//
// Table class for pengumuman
//
class cpengumuman extends cTable {
	var $nama_ruang;
	...
	function __construct() {
		...
		$this->TableVar = 'pengumuman';
		$this->TableName = 'pengumuman';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`pengumuman`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// nama_ruang
		$this->nama_ruang = new cField('pengumuman', 'pengumuman', 'x_nama_ruang', 'nama_ruang', '`nama_ruang`', '`nama_ruang`', 200, -1, FALSE, '`nama_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// nomor_pendaftaran
		$this->nomor_pendaftaran = new cField('pengumuman', 'pengumuman', 'x_nomor_pendaftaran', 'nomor_pendaftaran', '`nomor_pendaftaran`', '`nomor_pendaftaran`', 200, -1, FALSE, '`nomor_pendaftaran`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->nomor_peserta_ujian_sdmi = new cField('pengumuman', 'pengumuman', 'x_nomor_peserta_ujian_sdmi', 'nomor_peserta_ujian_sdmi', '`nomor_peserta_ujian_sdmi`', '`nomor_peserta_ujian_sdmi`', 200, -1, FALSE, '`EV__nomor_peserta_ujian_sdmi`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		$this->nomor_peserta_ujian_sdmi->Sortable = TRUE; // Allow sort
		$this->fields['nomor_peserta_ujian_sdmi'] = &$this->nomor_peserta_ujian_sdmi;

		// sekolah_asal
		$this->sekolah_asal = new cField('pengumuman', 'pengumuman', 'x_sekolah_asal', 'sekolah_asal', '`sekolah_asal`', '`sekolah_asal`', 200, -1, FALSE, '`sekolah_asal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sekolah_asal->Sortable = TRUE; // Allow sort
		$this->fields['sekolah_asal'] = &$this->sekolah_asal;

		// nama_lengkap
		$this->nama_lengkap = new cField('pengumuman', 'pengumuman', 'x_nama_lengkap', 'nama_lengkap', '`nama_lengkap`', '`nama_lengkap`', 200, -1, FALSE, '`nama_lengkap`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_lengkap->Sortable = TRUE; // Allow sort
		$this->fields['nama_lengkap'] = &$this->nama_lengkap;

		// jenis_kelamin
		$this->jenis_kelamin = new cField('pengumuman', 'pengumuman', 'x_jenis_kelamin', 'jenis_kelamin', '`jenis_kelamin`', '`jenis_kelamin`', 200, -1, FALSE, '`jenis_kelamin`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jenis_kelamin->Sortable = TRUE; // Allow sort
		$this->fields['jenis_kelamin'] = &$this->jenis_kelamin;

		// zona
		$this->zona = new cField('pengumuman', 'pengumuman', 'x_zona', 'zona', '`zona`', '`zona`', 200, -1, FALSE, '`zona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->zona->Sortable = TRUE; // Allow sort
		$this->fields['zona'] = &$this->zona;

		// nilai_akhir
		$this->nilai_akhir = new cField('pengumuman', 'pengumuman', 'x_nilai_akhir', 'nilai_akhir', '`nilai_akhir`', '`nilai_akhir`', 200, -1, FALSE, '`nilai_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// persyaratan
		$this->persyaratan = new cField('pengumuman', 'pengumuman', 'x_persyaratan', 'persyaratan', '`persyaratan`', '`persyaratan`', 200, -1, FALSE, '`persyaratan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->persyaratan->Sortable = TRUE; // Allow sort
		$this->persyaratan->OptionCount = 2;
		$this->fields['persyaratan'] = &$this->persyaratan;

		// catatan
		$this->catatan = new cField('pengumuman', 'pengumuman', 'x_catatan', 'catatan', '`catatan`', '`catatan`', 201, -1, FALSE, '`catatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->catatan->Sortable = TRUE; // Allow sort
		$this->fields['catatan'] = &$this->catatan;

		// keterangan
		$this->keterangan = new cField('pengumuman', 'pengumuman', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->keterangan->OptionCount = 2;
		$this->fields['keterangan'] = &$this->keterangan;

		// status
		$this->status = new cField('pengumuman', 'pengumuman', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`pengumuman`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `nopes` FROM `db_pd` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`nopes` = `pengumuman`.`nomor_peserta_ujian_sdmi` LIMIT 1) AS `EV__nomor_peserta_ujian_sdmi` FROM `pengumuman`" .
			") `EW_TMP_TABLE`";
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('nomor_pendaftaran', $rs))
				ew_AddFilter($where, ew_QuotedName('nomor_pendaftaran', $this->DBID) . '=' . ew_QuotedValue($rs['nomor_pendaftaran'], $this->nomor_pendaftaran->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`nomor_pendaftaran` = '@nomor_pendaftaran@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (is_null($this->nomor_pendaftaran->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@nomor_pendaftaran@", ew_AdjustSql($this->nomor_pendaftaran->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "pengumumanview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "pengumumanedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "pengumumanadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("pengumumanview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("pengumumanview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "pengumumanadd.php?" . $this->UrlParm($parm);
		else
			$url = "pengumumanadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("pengumumanedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("pengumumanadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("pengumumandelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "nomor_pendaftaran:" . ew_VarToJson($this->nomor_pendaftaran->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nomor_pendaftaran->CurrentValue)) {
			$sUrl .= "nomor_pendaftaran=" . urlencode($this->nomor_pendaftaran->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["nomor_pendaftaran"]))
				$arKeys[] = $_POST["nomor_pendaftaran"];
			elseif (isset($_GET["nomor_pendaftaran"]))
				$arKeys[] = $_GET["nomor_pendaftaran"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nomor_pendaftaran->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) {
			...
			if (!$Doc->ExportCustom) {
				...
				if ($this->keterangan->Exportable) $Doc->ExportAggregate($this->keterangan, 'COUNT');
	...
	function GetAutoFill($id, $val) {
		...
		if (preg_match('/^x(\d)*_nomor_peserta_ujian_sdmi$/', $id)) {
			$conn = &$this->Connection();
			$sSqlWrk = "SELECT `asal_sek` AS FIELD0, `nm_pes` AS FIELD1, `sex` AS FIELD2 FROM `db_pd`";
			...
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					...
					$this->jenis_kelamin->setDbValue($rs->fields[2]);

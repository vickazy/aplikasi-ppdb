...
$db_pd_add = NULL; // Initialize page object first

class cdb_pd_add extends cdb_pd {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_pd"] != "") {
				...
				$this->setKey("id_pd", $this->id_pd->CurrentValue); // Set up key
			} else {
				$this->setKey("id_pd", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "db_pdview.php")
	...
	function LoadDefaultValues() {
		...
		$this->id_pd->OldValue = $this->id_pd->CurrentValue;
		$this->nopes->CurrentValue = NULL;
		$this->nopes->OldValue = $this->nopes->CurrentValue;
		$this->nm_pes->CurrentValue = NULL;
		$this->nm_pes->OldValue = $this->nm_pes->CurrentValue;
		$this->tmp_lhr->CurrentValue = NULL;
		$this->tmp_lhr->OldValue = $this->tmp_lhr->CurrentValue;
		$this->tgl_lhr->CurrentValue = NULL;
		$this->tgl_lhr->OldValue = $this->tgl_lhr->CurrentValue;
		$this->tgl->CurrentValue = NULL;
		$this->tgl->OldValue = $this->tgl->CurrentValue;
		$this->bln->CurrentValue = NULL;
		$this->bln->OldValue = $this->bln->CurrentValue;
		$this->thn->CurrentValue = NULL;
		$this->thn->OldValue = $this->thn->CurrentValue;
		$this->sex->CurrentValue = NULL;
		$this->sex->OldValue = $this->sex->CurrentValue;
		$this->nm_ortu->CurrentValue = NULL;
		$this->nm_ortu->OldValue = $this->nm_ortu->CurrentValue;
		$this->asal_sek->CurrentValue = NULL;
		$this->asal_sek->OldValue = $this->asal_sek->CurrentValue;
		...
		$this->n_jml->CurrentValue = NULL;
		$this->n_jml->OldValue = $this->n_jml->CurrentValue;
	}
	...
	function NewRow() {
		...
		$row = array();
		$row['id_pd'] = $this->id_pd->CurrentValue;
		$row['nopes'] = $this->nopes->CurrentValue;
		$row['nm_pes'] = $this->nm_pes->CurrentValue;
		$row['tmp_lhr'] = $this->tmp_lhr->CurrentValue;
		$row['tgl_lhr'] = $this->tgl_lhr->CurrentValue;
		$row['tgl'] = $this->tgl->CurrentValue;
		$row['bln'] = $this->bln->CurrentValue;
		$row['thn'] = $this->thn->CurrentValue;
		$row['sex'] = $this->sex->CurrentValue;
		$row['nm_ortu'] = $this->nm_ortu->CurrentValue;
		$row['asal_sek'] = $this->asal_sek->CurrentValue;
		$row['n_ind'] = $this->n_ind->CurrentValue;
		$row['n_mat'] = $this->n_mat->CurrentValue;
		$row['n_ipa'] = $this->n_ipa->CurrentValue;
		$row['n_jml'] = $this->n_jml->CurrentValue;
		return $row;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// nopes
		$this->nopes->SetDbValueDef($rsnew, $this->nopes->CurrentValue, "", FALSE);

		// nm_pes
		$this->nm_pes->SetDbValueDef($rsnew, $this->nm_pes->CurrentValue, "", FALSE);

		// tmp_lhr
		$this->tmp_lhr->SetDbValueDef($rsnew, $this->tmp_lhr->CurrentValue, "", FALSE);

		// tgl_lhr
		$this->tgl_lhr->SetDbValueDef($rsnew, $this->tgl_lhr->CurrentValue, "", FALSE);

		// tgl
		$this->tgl->SetDbValueDef($rsnew, $this->tgl->CurrentValue, "", FALSE);

		// bln
		$this->bln->SetDbValueDef($rsnew, $this->bln->CurrentValue, "", FALSE);

		// thn
		$this->thn->SetDbValueDef($rsnew, $this->thn->CurrentValue, "", FALSE);

		// sex
		$this->sex->SetDbValueDef($rsnew, $this->sex->CurrentValue, "", FALSE);

		// nm_ortu
		$this->nm_ortu->SetDbValueDef($rsnew, $this->nm_ortu->CurrentValue, "", FALSE);

		// asal_sek
		$this->asal_sek->SetDbValueDef($rsnew, $this->asal_sek->CurrentValue, "", FALSE);

		// n_ind
		$this->n_ind->SetDbValueDef($rsnew, $this->n_ind->CurrentValue, 0, FALSE);

		// n_mat
		$this->n_mat->SetDbValueDef($rsnew, $this->n_mat->CurrentValue, 0, FALSE);

		// n_ipa
		$this->n_ipa->SetDbValueDef($rsnew, $this->n_ipa->CurrentValue, 0, FALSE);

		// n_jml
		$this->n_jml->SetDbValueDef($rsnew, $this->n_jml->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($db_pd_add)) $db_pd_add = new cdb_pd_add();
...
var CurrentForm = fdb_pdadd = new ew_Form("fdb_pdadd", "add");

// Validate form
fdb_pdadd.Validate = function() {
...
fdb_pdadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $db_pd_add->ShowPageHeader(); ?>
<?php
$db_pd_add->ShowMessage();
?>
<form name="fdb_pdadd" id="fdb_pdadd" class="<?php echo $db_pd_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($db_pd_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $db_pd_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="db_pd">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($db_pd_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($db_pd->nopes->Visible) { // nopes ?>
	<div id="r_nopes" class="form-group">
		<label id="elh_db_pd_nopes" for="x_nopes" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->nopes->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->nopes->CellAttributes() ?>>
...
<?php if ($db_pd->nm_pes->Visible) { // nm_pes ?>
	<div id="r_nm_pes" class="form-group">
		<label id="elh_db_pd_nm_pes" for="x_nm_pes" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->nm_pes->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->nm_pes->CellAttributes() ?>>
...
<?php if ($db_pd->tmp_lhr->Visible) { // tmp_lhr ?>
	<div id="r_tmp_lhr" class="form-group">
		<label id="elh_db_pd_tmp_lhr" for="x_tmp_lhr" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->tmp_lhr->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->tmp_lhr->CellAttributes() ?>>
...
<?php if ($db_pd->tgl_lhr->Visible) { // tgl_lhr ?>
	<div id="r_tgl_lhr" class="form-group">
		<label id="elh_db_pd_tgl_lhr" for="x_tgl_lhr" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->tgl_lhr->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->tgl_lhr->CellAttributes() ?>>
...
<?php if ($db_pd->tgl->Visible) { // tgl ?>
	<div id="r_tgl" class="form-group">
		<label id="elh_db_pd_tgl" for="x_tgl" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->tgl->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->tgl->CellAttributes() ?>>
...
<?php if ($db_pd->bln->Visible) { // bln ?>
	<div id="r_bln" class="form-group">
		<label id="elh_db_pd_bln" for="x_bln" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->bln->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->bln->CellAttributes() ?>>
...
<?php if ($db_pd->thn->Visible) { // thn ?>
	<div id="r_thn" class="form-group">
		<label id="elh_db_pd_thn" for="x_thn" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->thn->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->thn->CellAttributes() ?>>
...
<?php if ($db_pd->sex->Visible) { // sex ?>
	<div id="r_sex" class="form-group">
		<label id="elh_db_pd_sex" for="x_sex" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->sex->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->sex->CellAttributes() ?>>
...
<?php if ($db_pd->nm_ortu->Visible) { // nm_ortu ?>
	<div id="r_nm_ortu" class="form-group">
		<label id="elh_db_pd_nm_ortu" for="x_nm_ortu" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->nm_ortu->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->nm_ortu->CellAttributes() ?>>
...
<?php if ($db_pd->asal_sek->Visible) { // asal_sek ?>
	<div id="r_asal_sek" class="form-group">
		<label id="elh_db_pd_asal_sek" for="x_asal_sek" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->asal_sek->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->asal_sek->CellAttributes() ?>>
...
<?php if ($db_pd->n_ind->Visible) { // n_ind ?>
	<div id="r_n_ind" class="form-group">
		<label id="elh_db_pd_n_ind" for="x_n_ind" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->n_ind->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->n_ind->CellAttributes() ?>>
...
<?php if ($db_pd->n_mat->Visible) { // n_mat ?>
	<div id="r_n_mat" class="form-group">
		<label id="elh_db_pd_n_mat" for="x_n_mat" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->n_mat->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->n_mat->CellAttributes() ?>>
...
<?php if ($db_pd->n_ipa->Visible) { // n_ipa ?>
	<div id="r_n_ipa" class="form-group">
		<label id="elh_db_pd_n_ipa" for="x_n_ipa" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->n_ipa->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->n_ipa->CellAttributes() ?>>
...
<?php if ($db_pd->n_jml->Visible) { // n_jml ?>
	<div id="r_n_jml" class="form-group">
		<label id="elh_db_pd_n_jml" for="x_n_jml" class="<?php echo $db_pd_add->LeftColumnClass ?>"><?php echo $db_pd->n_jml->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $db_pd_add->RightColumnClass ?>"><div<?php echo $db_pd->n_jml->CellAttributes() ?>>
...
<?php if (!$db_pd_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $db_pd_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $db_pd_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->

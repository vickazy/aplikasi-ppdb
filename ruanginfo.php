...
class cruang extends cTable {
	...
	function __construct() {
		...
		// id_ruang
		$this->id_ruang = new cField('ruang', 'ruang', 'x_id_ruang', 'id_ruang', '`id_ruang`', '`id_ruang`', 3, -1, FALSE, '`id_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_ruang->Sortable = TRUE; // Allow sort
		$this->id_ruang->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_ruang'] = &$this->id_ruang;

		// nama_ruang
		$this->nama_ruang = new cField('ruang', 'ruang', 'x_nama_ruang', 'nama_ruang', '`nama_ruang`', '`nama_ruang`', 200, -1, FALSE, '`nama_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_ruang->Sortable = TRUE; // Allow sort
		$this->fields['nama_ruang'] = &$this->nama_ruang;

		// pwd
		$this->pwd = new cField('ruang', 'ruang', 'x_pwd', 'pwd', '`pwd`', '`pwd`', 200, -1, FALSE, '`pwd`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pwd->Sortable = TRUE; // Allow sort
		$this->fields['pwd'] = &$this->pwd;

		// user_level
		$this->user_level = new cField('ruang', 'ruang', 'x_user_level', 'user_level', '`user_level`', '`user_level`', 3, -1, FALSE, '`user_level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_ruang->setDbValue($conn->Insert_ID());
			$rs['id_ruang'] = $this->id_ruang->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_ruang', $rs))
				ew_AddFilter($where, ew_QuotedName('id_ruang', $this->DBID) . '=' . ew_QuotedValue($rs['id_ruang'], $this->id_ruang->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`id_ruang` = @id_ruang@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_ruang->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_ruang->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_ruang@", ew_AdjustSql($this->id_ruang->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "ruangedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "ruangadd.php")
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_ruang:" . ew_VarToJson($this->id_ruang->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_ruang->CurrentValue)) {
			$sUrl .= "id_ruang=" . urlencode($this->id_ruang->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["id_ruang"]))
				$arKeys[] = $_POST["id_ruang"];
			elseif (isset($_GET["id_ruang"]))
				$arKeys[] = $_GET["id_ruang"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_ruang->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->id_ruang->setDbValue($rs->fields('id_ruang'));
	...
	// Common render codes
		...
		// id_ruang
		$this->id_ruang->LinkCustomAttributes = "";
		$this->id_ruang->HrefValue = "";
		$this->id_ruang->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// id_ruang
		$this->id_ruang->EditAttrs["class"] = "form-control";
		$this->id_ruang->EditCustomAttributes = "";
		$this->id_ruang->EditValue = $this->id_ruang->CurrentValue;
		$this->id_ruang->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->id_ruang->Exportable) $Doc->ExportCaption($this->id_ruang);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->id_ruang->Exportable) $Doc->ExportField($this->id_ruang);
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`nama_ruang` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING, EW_USER_TABLE_DBID);
		$sParentUserIDFilter = '`nama_ruang` IN (SELECT `nama_ruang` FROM ' . "`ruang`" . ' WHERE `nama_ruang` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING, EW_USER_TABLE_DBID) . ')';
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		...
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `ruang`";
